...
// ===== Animate =====
function animateBlocks(){ blocks.forEach(b=>{ b.y+=b.speed; if(b.y>canvas.height)b.y=-b.size; ctx.drawImage(b.img,b.x,b.y,b.size,b.size); }); }
function animateParticles(){
    particles.forEach(p=>{
        p.y-=p.speed;
        if(p.y<0)p.y=canvas.height;
        if(mouse.x&&mouse.y){ const dx=mouse.x-p.x; const dy=mouse.y-p.y; const d=Math.sqrt(dx*dx+dy*dy); if(d<100){ p.x+=dx*0.01; p.y+=dy*0.01; } }
        ctx.fillStyle=`rgba(255,255,255,${p.alpha})`;
        ctx.beginPath(); ctx.arc(p.x,p.y,p.size,0,Math.PI*2); ctx.fill();
    });
}
function animate(){ ctx.clearRect(0,0,canvas.width,canvas.height); animateBlocks(); animateParticles(); requestAnimationFrame(animate); }
animate();
...
async function fetchServerStats(){
    ...
    }catch(err){ console.error('Failed to fetch server stats:',err); document.getElementById('players').textContent='Offline'; }
}
